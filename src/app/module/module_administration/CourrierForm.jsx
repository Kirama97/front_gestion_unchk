...
const CourrierForm = ({ onSave, editingDoc, setEditingDoc }) => {
  ...
  useEffect(() => {
    if (editingDoc) {
      setFormData({
        ...
        type: editingDoc.type || 'COURRIER_ARRIVE',
        cheminFichier: editingDoc.cheminFichier || ''
      })
    } else {
      setFormData({ titre: '', description: '', type: 'COURRIER_ARRIVE', cheminFichier: '' })
    }
  }, [editingDoc])
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      showToast("Courrier téléversé avec succès !", "success")
    } catch (err) {
      console.error(err)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDoc) {
        ...
        showToast("Courrier mis à jour avec succès.", "success")
        setEditingDoc(null)
      } else {
        await apiPost('/api/documents', formData)
        showToast("Courrier enregistré avec succès.", "success")
      }
      setFormData({ titre: '', description: '', type: 'COURRIER_ARRIVE', cheminFichier: '' })
      if (onSave) onSave()
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-5 border border-slate-100 rounded-2xl shadow-sm flex flex-col gap-4">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-sm font-bold text-slate-800">
            {editingDoc ? "Modifier le Courrier" : "Enregistrer un Courrier"}
          </h3>
          <p className="text-[11px] text-slate-400">Gérez la correspondance administrative.</p>
        </div>
        {editingDoc && (
          <button
            type="button"
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Objet du Courrier</label>
        <input
          type="text"
          name="titre"
          value={formData.titre}
          onChange={handleChange}
          placeholder="Ex: Demande de stage Académique"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Type de Courrier</label>
          <select
            name="type"
            value={formData.type}
            onChange={handleChange}
          ...
          >
            <option value="COURRIER_ARRIVE">Courrier Arrivé</option>
            <option value="COURRIER_DEPART">Courrier Départ</option>
          </select>
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Document (Scanné)</label>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Description / Commentaires</label>
        <textarea
          name="description"
          value={formData.description}
          onChange={handleChange}
          placeholder="Détails du courrier ou annotations..."
          rows="3"
      ...
      >
        ...
        <span>{editingDoc ? "Mettre à jour le Courrier" : "Enregistrer le Courrier"}</span>
      </button>
    </form>
  )
}
