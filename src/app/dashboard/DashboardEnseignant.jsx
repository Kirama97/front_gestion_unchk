    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          <span className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-bold text-lg shadow-md shadow-emerald-600/20">
            U
          </span>
          <div>
            <h1 className="text-lg font-bold text-slate-800 tracking-tight">UNCHK Portal</h1>
            <p className="text-[10px] text-emerald-600 font-semibold tracking-wider uppercase">Espace Enseignant</p>
          </div>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block">
            <p className="text-xs font-semibold text-slate-800">{user.email || 'Enseignant'}</p>
            <p className="text-[10px] text-slate-400 capitalize">{user.role || 'Enseignant'}</p>
          </div>
          <button
        ...
        <aside className="w-64 bg-white border-r border-slate-200 p-6 flex flex-col gap-6 max-md:hidden">
          <div className="flex flex-col gap-1">
            ...
            <Link
              ...
              className="text-sm font-medium text-slate-700 hover:text-emerald-600 hover:bg-emerald-50/50 px-3 py-2.5 rounded-lg transition-all duration-200 flex items-center gap-2.5"
            >
              📅 Emploi du Temps
            </Link>
