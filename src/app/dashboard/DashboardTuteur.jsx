...
const DashboardTuteur = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (!isFirstLoad.current) {
        ...
        msgs.forEach(m => {
          if (!m.lu && !previousMessagesRef.current.some(prev => prev.id === m.id)) {
            showToast(`Nouveau message de ${m.titre || 'Utilisateur'} : ${m.description}`, 'success')
          }
        })
      } else {
      ...
      console.error('Error fetching notifications for tutor:', err)
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans antialiased text-slate-800">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/80 backdrop-blur-md shadow-sm transition-all duration-300">
        <div className="flex h-12 items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/tuteur/emploi-du-temps" className="flex items-center gap-3 hover:opacity-95 transition-opacity">
              <div className="relative flex h-8 w-8 items-center justify-center rounded-md bg-indigo-600 shadow-md shadow-indigo-500/20">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] font-bold tracking-wider text-indigo-600 uppercase">Espace Tuteur</span>
              </div>
            </Link>
          </div>
              ...
              {isMessagesOpen && (
                <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-slate-200/80 bg-white p-2 shadow-xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-3 duration-200">
                  ...
                  <div className="mt-1 max-h-60 overflow-y-auto space-y-1">
                    {messages.length === 0 ? (
                      <div className="text-center py-6 text-slate-400 text-xs font-medium">Aucun message</div>
                    ) : (
                      messages.map((msg) => (
                        <div 
                          key={msg.id} 
                          onClick={() => handleMessageClick(msg)}
                          className={`flex gap-3 items-start p-2.5 rounded-xl transition cursor-pointer hover:bg-indigo-50/40 ${!msg.lu ? 'bg-indigo-50/40' : ''}`}
                        >
                          <div className="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-xs">
                          ...
                          {!msg.lu && <span className="w-2 h-2 rounded-full bg-indigo-500 self-center"></span>}
                        </div>
                      ))
                    )}
                  </div>
                ...
                <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-slate-200/80 bg-white p-2 shadow-xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-3 duration-200">
                  ...
                  <div className="mt-1 max-h-60 overflow-y-auto space-y-1">
                    {notifications.length === 0 ? (
                      <div className="text-center py-6 text-slate-400 text-xs font-medium">Aucune notification</div>
                    ) : (
                      notifications.map((notif) => (
                        <div 
                          key={notif.id} 
                          onClick={() => handleNotificationClick(notif)}
                          className={`flex gap-3 items-start p-2.5 rounded-xl transition cursor-pointer hover:bg-slate-50 ${!notif.lu ? 'bg-indigo-50/40' : ''}`}
                        >
                          <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs bg-indigo-100 text-indigo-700`}>
                          ...
                          {!notif.lu && <span className="w-2 h-2 rounded-full bg-indigo-500 self-center"></span>}
                        </div>
                      ))
                    )}
                  </div>
              ...
              {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-200/80 bg-white p-1.5 shadow-xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-3 duration-200">
                  <div className="px-3 py-2 border-b border-slate-100">
                    <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Espace Tuteur</p>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white p-6 shadow-2xl transition-transform duration-300 transform md:hidden ${
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-md bg-indigo-600 shadow-md shadow-indigo-500/20">
              <span className="text-white font-extrabold text-lg">U</span>
            </div>
            <div className="flex flex-col">
